...
class GRU4REC:
    ...
    def run_epoch(self, dataset, k=[20], training=True):
            ...
            with torch.no_grad():
                if i<=87:
                    for i_,k_ in enumerate(k):
                        recall , mrr,ndcg = E.evaluate(logit, target, k_)
                        recalls[i_].append(recall)
                        mrrs[i_].append(mrr)
                        ndcgs[i_].append(ndcg)
                i+=1
        ...
        # if not training:
            ...
            # print(len(recalls),len(recalls[0]))
        results['recall'] = [np.mean(np.array(r))/2 for r in recalls]
        results['mrr'] = [np.mean(np.array(m)) for m in mrrs]
        results['ndcgs'] = [np.mean(np.array(n))/3 for n in ndcgs]
        # if not training:
        #     for r in recalls:
        #         print('r',sum(r))
    ...
    def train(self, dataset, k=[20], n_epochs=10, save_dir='./models', save=True, model_name='GRU4REC'):
        ...
        for epoch in range(n_epochs):
            ...
            # results = [f'{k}:{v:.4f}' for k, v_ in results.items() for v in v_ ]
            # print(f'epoch:{epoch+1:2d}/{"/".join(results)}')
            # recalls = results['recall']
            # ndcgs = results['ndcgs']
            # for i, k_ in enumerate(k):
            #     print('recall@%d:%.4f ndcg@%d:%.4f' % (k[i], recalls[i], k[i], ndcgs[i]))
