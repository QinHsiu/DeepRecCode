...
def test_item_based_cf():
    cf_based_similarity  =  ItemCFBasedSimilarity(data_file='./data/Sports_and_Outdoors.txt',
                                similarity_path='./data/item_cf_iuf_similarity.pkl',
                                model_type='ItemCF_IUF')
    print(cf_based_similarity.most_similar('10000', top_k=4))
    test_batch=[[random.randint(1,1000) for _ in range(256)] for _ in range(256)]
    re_batch=[[0 for _ in range(256)] for _ in range(256)]
    for r in range(len(test_batch)):
        for c in range(len(test_batch[r])):
            cur=test_batch[r][0]
            if test_batch[r][c]==test_batch[r][0]:
                re_batch[r][c] = 1
            if str(cur) in cf_based_similarity.similarity_dict:
                temp=map(int,cf_based_similarity.most_similar(str(cur), top_k=1000))
                if test_batch[r][c] in temp:
                    re_batch[r][c]=1
