...
def Amazon(dataset_name, rating_score):
    ...
    datas = []
    # older Amazon
    data_flie = './path/ml_1m_ratings.dat'
    # latest Amazon
    with open(data_flie,"r+") as fr:
        lines=fr.readlines()
...
def Amazon_meta(dataset_name, data_maps):
    ...
    datas = {}
    meta_infos = {}
    meta_file = './path/ml_1m_movies.dat'
    item_ids = list(data_maps['item2id'].keys())
    with open(meta_file,"rb") as fr:
        lines=fr.readlines()
    for line in tqdm.tqdm(lines):
        # info = json.loads(line)
        temp=str(line).split("::")
        id,name,category=temp
        info={}
        asin=None
        ...
        #
        # # print(info)
        if info["asin"] not in item_ids:
             continue
        meta_infos[info["asin"]]=info
    return meta_infos
...
def main(data_name, data_type='Amazon'):
    ...
    if data_type == 'Yelp':
        ...
        datas = Amazon(data_name+'_0', rating_score=rating_score)
...
def ML_1M():
    ...
    item_core = 5
    datas = []
    data_file = './path/ml_1m_ratings.dat'
    lines = open(data_file).readlines()
    for line in tqdm.tqdm(lines[0:]):
        user, item, attribute, timestamp = line.strip().split('::')
        datas.append((user, item, int(timestamp)))
    ...
    # attribute_file = './path/artist2attributes.json'
    # meta_item2attribute = json.loads(open(attribute_file).readline())

    meta_infos = Amazon_meta('ml-1m', data_maps)
    _, _, _, meta_item2attribute = get_attribute_Amazon(meta_infos, data_maps,0)
