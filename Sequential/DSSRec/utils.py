...
def get_args():
    ...
    parser.add_argument('--ckp', default=24, type=int, help="pretrain epochs 10, 20, 30...")
...
def get_pretrain_args(parser):
    # pre train args
    parser.add_argument("--pre_epochs", type=int, default=20, help="number of pre_train epochs")
...
def get_user_seqs_csv_(data_file):
...
def get_user_seqs_long_csv_(data_file):
    """
...
def get_user_seqs_long_csv(data_file):
    ...
    """
    data_df = open(data_file,"r+").readlines()
    user_seq = []
    long_sequence = []
    item_set = set()
    user_set = set()
    ...
    max_item = len(item_set)
    print('Number of unique item ids', max_item)
    # print(len(user_seq),len(user_set))
    assert len(user_set) == len(user_seq)
    return user_seq, max_item, long_sequence

def get_user_seqs_csv(data_file):
    user_seq,max_item,_=get_user_seqs_long_csv(data_file)
    num_users = len(user_seq)
    num_items = max_item + 2
    valid_rating_matrix = generate_rating_matrix_valid(user_seq, num_users, num_items)
    test_rating_matrix = generate_rating_matrix_test(user_seq, num_users, num_items)
