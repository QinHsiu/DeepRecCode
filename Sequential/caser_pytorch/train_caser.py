...
class Recommender(object):
    ...
    def fit(self, train, test, verbose=False):
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            t2 = time()
            k = [1, 5, 10, 20]
            if verbose and (epoch_num + 1) % self._n_iter == 0:
                precision, recall, mean_aps,ndcgs = evaluate_ranking(self, test,None, k)
                for v in range(len(k)):
                    print('HIT@%d: %.4f, NDCG@%d: %.4f' % (k[v], np.mean(recall[v]), k[v], np.mean(ndcgs[v])))

                # output_str = "Epoch %d [%.1f s]\tloss=%.4f, map=%.4f, " \
                #              "pre
                #
                #
                #              c@1=%.4f, prec@5=%.4f, prec@10=%.4f, " \
...
def main(file):
    ...
    # data arguments
    parser.add_argument('--train_root', type=str, default=file+'/train.txt')
    parser.add_argument('--test_root', type=str, default=file+'/test.txt')
    parser.add_argument('--L', type=int, default=50) # sequence length
    parser.add_argument('--T', type=int, default=1) # target length
    # train arguments
    parser.add_argument('--n_iter', type=int, default=1) # 50 #迭代次数
    parser.add_argument('--seed', type=int, default=2021)
    parser.add_argument('--batch_size', type=int, default=256) # 512 #batch大小
    parser.add_argument('--learning_rate', type=float, default=1e-1)# 1e-3
    parser.add_argument('--l2', type=float, default=1e-6) # 1e-6
    parser.add_argument('--neg_samples', type=int, default=3) # 3 #负采样
    ...
    model_parser.add_argument('--d', type=int, default=64) #50
    model_parser.add_argument('--nv', type=int, default=4) #垂直过滤

    model_parser.add_argument('--nh', type=int, default=16) #水平过滤
    model_parser.add_argument('--drop', type=float, default=0.1) #0.5 丢弃率
    model_parser.add_argument('--ac_conv', type=str, default='relu') #卷积层激活函数
    model_parser.add_argument('--ac_fc', type=str, default='relu') #全连接层激活函数
    ...
    show_args_info(config)
    print("--------------------Configure Info:------------")
    show_args_info(model_config)
...
if __name__ == '__main__':
    ...
    for f in file_names:
        if os.path.isdir(os.getcwd()+"/data/"+f):
            file=os.getcwd()+"/data/"+f
            print(file)
            if f=="ml-1m":
                main(file)
            # file=os.getcwd()+"/data/"+f
