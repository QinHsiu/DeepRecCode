...
def setup_train(args):
    ...
    # 调用create_experiment_export_folder函数
...
def create_optimizer(model, args):
    #back propagate only when requires_grad = True
    if args.optimizer == 'Adam':
        return optim.Adam(filter(lambda p:p.requires_grad,model.parameters()), lr=args.lr, weight_decay=args.weight_decay)

    return optim.SGD(filter(lambda p:p.requires_grad,model.parameters()), lr=args.lr, weight_decay=args.weight_decay, momentum=args.momentum)
